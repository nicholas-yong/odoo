...
class SupportTicketController(http.Controller):
    ...
    def support_process_ticket(self, **kwargs):
        ...
        create_dict = {
                ...
                'category_id': int(values['category']), 
    ...
    def support_ticket_close(self, **kw):
        ...
        #As per Madhu, redirect to a new page telling the user that they have closed the ticket.
        return werkzeug.utils.redirect("/support/ticket/close/thanks")

    @http.route('/support/ticket/close/thanks', type="http", auth="public", website=True)
    def support_ticket_close_thanks(self, **kw):
        """Displays a thank you page after the user closes a ticket"""
        return http.request.render('website_support.support_close_thank_you',{})
