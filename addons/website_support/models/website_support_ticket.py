...
class WebsiteSupportTicket(models.Model):
            ...
            for sla_alert in self.env['website.support.sla.alert'].search([('vsa_id','=',active_sla_ticket.sla_id.id), ('alert_time','>=', active_sla_ticket.sla_timer)]):
                ...
                if sla_alert not in active_sla_ticket.sla_alert_ids:
                    ...
                    for my_user in active_sla_ticket.category_id.cat_user_ids:
                        values = notification_template.generate_email(active_sla_ticket.id, ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])
    ...
    def request_approval(self):
        ...
        #dict_fields =[]
        #values = self.env['mail.compose.message'].generate_email_for_composer(approval_email.id, [self.id], dict_fields)[self.id]

        request_message = self.env['mail.render.mixin']._render_template(approval_email['body_html'], 'website.support.ticket', [self.id], post_process=True)[self.id]
    ...
    #def delete_ticket(self):
     #   ticket = self.env['website.support.ticket'].browse(self.id)
     #   ticket.unlink()
        #Once the unlink is complete, we probably want to go back towards the main form.
     #   return
     #   {
           # 'name': 'website.support.ticket tree view',
          #  'view_type': 'form',
         #   'view_mode': 'list',
    ...
    def create(self, vals):
        ...
        ticket_open_email_template.send_mail( int(new_id.id), True)
        ...
        for my_user in new_id.category_id.cat_user_ids:
            values = notification_template.generate_email(new_id.id, ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])
    ...
    def write(self, values, context=None):
        ...
        if 'user_id' in values:
            ...
            email_values = email_template.generate_email([self.id], ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])[self.id]
    ...
    def send_survey(self):
        ...
        #values = notification_template.generate_email(self.id, notification_template._values )
        send_mail = notification_template.send_mail( self.id, True )
...
class WebsiteSupportTicketMessage(models.Model):
    ...
    def create(self, values):
        ...
        if values['by'] == "customer":
            ...
            email_values = customer_reply_email_template.generate_email(new_record.id, ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])
...
class WebsiteSupportTicketClose(models.TransientModel):
    ...
    def _onchange_template_id(self):
        if self.template_id:
            values = self.env['mail.compose.message'].generate_email_for_composer(self.template_id.id, [self.ticket_id.id], ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'] )
            self.message = values[self.ticket_id.id]['body']
...
class WebsiteSupportTicketCompose(models.Model):
    ...
    def _onchange_template_id(self):
        if self.template_id:
            values = self.env['mail.compose.message'].generate_email_for_composer(self.template_id.id, [self.ticket_id.id],['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'] ) 
            self.body = values[self.ticket_id.id]['body']
    ...
    def send_reply(self):
        ...
        # We probably want this to work even without a template
        if setting_staff_reply_email_template_id:
            ...
            # Replace base email wrapper fields with support ticket model fields
            values = email_wrapper.generate_email([self.id], ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])
            new_dict = values[self.id]
            #new_list = list(values[self.id].items()
            new_dict['model'] = "website.support.ticket"
            new_dict['res_id'] = self.ticket_id.id
            send_mail = self.env['mail.mail'].create(new_dict)
            send_mail.send()
